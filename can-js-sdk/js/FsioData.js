define(["FsioBase"], function(FsioBase) {
     ...
     * @see Fsio#data
     */
    function FsioData(host, version, dac, headers) {
        this.init(host, "data", version, dac, headers);
    }
     ...
     * @param {integer} upload_id  Upload ID received from
     * {@link #partialUploadInit}.
     * @param {string|Blob} data   Data to upload, size must match the
    ...
    FsioData.prototype.uploadPartially = function(
        ...
        if(data instanceof Blob) {
            data.slice = data.slice || data.webkitSlice || data.mozSlice;
            data_len = data.size;
        } else {
            // String
        ...
        (function loop(self) {
            ...
            self.partialUploadQuery(token, upload_id, function(
                ...
                if(jqXHR.status != 200) {
                    ...
                    var range = "bytes " + start_pos + "-" + (end_pos-1) + "/" +
                        data_len;
                    console.log("Uploading with id " + upload_id +
                                  ", range: " + range);
                    hdrs["content-range"] = range;
                    // upload next part
                    var part = data instanceof Blob ?
                        data.slice(start_pos, end_pos, data.type) :
                        data.substring(start_pos, end_pos);
                    self.partialUpload(token, upload_id, part, function(jqXHR, textStatus) {
                        // done or error?
    ...
    FsioData.prototype.download = function(
        ...
        // do simple CORS request
        this.request("GET", "files/" + object_name, complete, params, headers);
    };
